...
boxoffice as (
    ...
    where revenue_worldwide_usd is not null  -- Only keep if a match was found
),
...
merged as (
    select
        ...
        coalesce(b.revenue_domestic_usd, 0) as revenue_domestic_usd,
        coalesce(b.revenue_foreign_usd, 0) as revenue_foreign_usd,
        m.budget_usd,

        -- Vote data (only from TMDB)
        m.vote_avg,
        m.vote_count
